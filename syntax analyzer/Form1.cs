...
namespace compilerDesign
{
	public partial class Form1 : Form
	{
		...
		//Returns a list of matches strings
		...
		//Takes a list of matching strings, it gets the datatype of each string and stores it in the list of Nodes (Token)
		List<Token> Scanner( List<string> splitCode ) {
			List<Token> output = new List<Token>();
			List<string> identifiers = new List<string>( new string[] { "int", "float", "string", "double", "bool", "char", "var"} );
			...
			//Checks where it's a valid variable or not
		...
		/*
		 has a main “for” loop that iterates over the given list of tokens, 
		it test which of the three rules will start accepting the i-th token, 
		once a rule starts accepting, this rule becomes the selected rule, it 
		will remain selected until it either returns an error or returns an “Ok”,
		which means the last sequence of tokens did follow the rule correctly
		 */
